...
function weightedRandomActivity() {
  ...
  for (const [name, data] of entries) { if ((r -= data.weight) <= 0) return name; }
  return 'INACTIVE';
}
...
function newUser(username) {
  ...
  return { username, createdAt: Date.now(), updatedAt: Date.now(), skills, totalLevel: totalLevel(skills), totalXP: totalXP(skills), activity: 'INACTIVE', needsHpMigration: false, version: 1 };
}
...
// Fetch all users from KV (beware: unbounded scan). Uses simple batching for parallel value retrieval per page.
async function getAllUsers(env) {
  ...
  do {
    ...
    // Batch fetch in chunks to avoid overwhelming KV (Cloudflare suggests moderate concurrency)
    const chunkSize = 50;
...
async function handleLeaderboard(env, url) {
  ...
  // Hard cap to avoid excessively huge payloads accidentally (frontend currently consumes full set for search)
  const HARD_CAP = 5000; // adjustable
...
async function router(request, env) {
  ...
  if (method === 'OPTIONS') return new Response(null, { headers: { 'access-control-allow-origin': '*', 'access-control-allow-methods': 'GET,POST,OPTIONS' }});
  return notFound();
}
